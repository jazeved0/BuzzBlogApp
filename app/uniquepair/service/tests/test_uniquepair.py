...
class TestService(unittest.TestCase):
  def test_add_and_get(self):
    with UniquepairClient(IP_ADDRESS, PORT) as client:
      ...
      uniquepair = client.add(TRequestMetadata(id="1"), "test_add_and_get",
      ...
      gotten_uniquepair = client.get(TRequestMetadata(id="2"), uniquepair.id)
  ...
  def test_find(self):
    with UniquepairClient(IP_ADDRESS, PORT) as client:
      ...
      first_elem = random.randint(1, 2 ** 16)
      second_elem = random.randint(1, 2 ** 16)
      uniquepair = client.add(TRequestMetadata(id="1"), "test_find", first_elem,
          second_elem)
      # Find that uniquepair and check its id.
      found_uniquepair = client.find(TRequestMetadata(id="2"), "test_find",
          first_elem, second_elem)
  ...
  def test_fetch(self):
    with UniquepairClient(IP_ADDRESS, PORT) as client:
      ...
      uniquepairs = []
      for i in range(10):
        uniquepairs.append(client.add(TRequestMetadata(id="1"), "test_fetch",
            random.randint(1, 2 ** 16), random.randint(1, 2 ** 16)))
      ...
      fetched_uniquepairs = client.fetch(TRequestMetadata(id="2"),
      ...
      uniquepair = client.add(TRequestMetadata(id="3"), "test_fetch", 0,
          random.randint(1, 2 ** 16))
      # Fetch uniquepairs whose first element is 0 and check their ids.
      fetched_uniquepairs = client.fetch(TRequestMetadata(id="4"),
      ...
      uniquepair = client.add(TRequestMetadata(id="5"), "test_fetch",
          random.randint(1, 2 ** 16), 0)
      # Fetch uniquepairs whose second element is 0 and check their ids.
      fetched_uniquepairs = client.fetch(TRequestMetadata(id="6"),
  ...
  def test_count(self):
    with UniquepairClient(IP_ADDRESS, PORT) as client:
      # Add 10 random uniquepairs.
      for i in range(10):
        client.add(TRequestMetadata(id="1"), "test_count",
            random.randint(1, 2 ** 16), random.randint(1, 2 ** 16))
      # Add a uniquepair whose first element is 0.
      client.add(TRequestMetadata(id="2"), "test_count", 0,
          random.randint(1, 2 ** 16))
      # Add a uniquepair whose second element is 0.
      client.add(TRequestMetadata(id="3"), "test_count",
          random.randint(1, 2 ** 16), 0)
      # Count uniquepairs whose first element is 0.
      self.assertEqual(1, client.count(TRequestMetadata(id="4"),
          TUniquepairQuery(domain="test_count", first_elem=0)))
      # Count uniquepairs whose second element is 0.
      self.assertEqual(1, client.count(TRequestMetadata(id="5"),
          TUniquepairQuery(domain="test_count", second_elem=0)))
