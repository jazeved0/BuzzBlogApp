...
from buzzblog.gen.buzzblog.ttypes import *


class ThriftClientFactory:
  def __init__(self):
    topology_filename = "/usr/local/etc/buzzblog/topology.yml"
    with open(topology_filename) as topology_file:
      topology = yaml.safe_load(topology_file)
      self._account_servers = topology["account"]["service"]
      self._follow_servers = topology["follow"]["service"]
      self._like_servers = topology["like"]["service"]
      self._post_servers = topology["post"]["service"]
