...
class Client(Iface):
    ...
    def fetch(self, query, limit, offset):
        ...
        """
        self.send_fetch(query, limit, offset)
        return self.recv_fetch()

    def send_fetch(self, query, limit, offset):
...
class Processor(Iface, TProcessor):
    ...
    def process_fetch(self, seqid, iprot, oprot):
        ...
        result = fetch_result()
        try:
            result.success = self._handler.fetch(args.query, args.limit, args.offset)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
...
fetch_args.thrift_spec = (
    None,  # 0
    (1, TType.STRUCT, 'query', [TUniquepairQuery, None], None, ),  # 1
